...
class Kmean:
    def __init__(self ,file, cluster_num , max_itr):
        ...
        self.pure_data = self.cdata[:,np.array([x for x in range(0,self.data.shape[1]-1)])]
        self.C =[]

        #for i in range(1,data.shape[0]):
        #    if  self.data[i,self.data.shape[1]-1] not in self.data_class:
        #        self.data_class[self.data[i,self.data.shape[1]-1]] = self.pure_data[i-1,:] 
        #    self.data_class[self.data[i,self.data.shape[1]-1]].append(self.pure_data[i-1,:])
    ...
    def centerOfMass(self , cluster_data_arr):
        ...
        data_arr = cluster_data_arr[:,np.array([x for x in range(0,cluster_data_arr.shape[1]-1)])]
...
#def main():
#    model = Kmean('datasets/iris.csv',3,10)
#    model.train();
#    model.printClusters();
